...
export function WorkshopSettings() {
  ...
  const saveStages = async () => {
    ...
    if (bad) { setStagesError('Sila isi semua nama peringkat.'); return }
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-8 space-y-4">
      ...
      <form onSubmit={handleSave} className="bg-surface-card border border-hairline rounded-lg p-5 space-y-4">
        ...
        <div>
          ...
          <input value={name} onChange={e => setName(e.target.value)} required
            placeholder="cth: Bengkel Maju Jaya" className={inputCls} />
        </div>
        ...
        {saved && <p className="text-badge-success text-xs bg-green-50 border border-green-200 rounded-md px-3 py-2">Tetapan disimpan berjaya!</p>}
      ...
      <div className="bg-surface-card border border-hairline rounded-lg p-5">
        <div className="mb-4">
          <h2 className="font-semibold text-ink text-sm">Peringkat Kerja</h2>
          <p className="text-xs text-mute mt-0.5">Susun aliran kerja bengkel anda. Peringkat pertama = terima, terakhir = siap.</p>
        </div>
          ...
          {stages.map((s, i) => {
            ...
            return (
              <div key={s.value} className="flex items-center gap-2">
                ...
                <input
                  value={s.short}
                  onChange={e => updateShort(i, e.target.value.slice(0, 8))}
        ...
        <button type="button" onClick={addStage}
          ...
          <Plus className="w-4 h-4" /> Tambah Peringkat
        </button>

        {stagesError && <p className="text-red-600 text-xs bg-red-50 border border-red-200 rounded-md px-3 py-2 mb-3">{stagesError}</p>}
        {stagesSaved && <p className="text-badge-success text-xs bg-green-50 border border-green-200 rounded-md px-3 py-2 mb-3">Peringkat disimpan berjaya!</p>}
        ...
        <button type="button" onClick={saveStages} disabled={stagesSaving}
          ...
          {stagesSaving ? 'Menyimpan...' : 'Simpan Peringkat'}
        </button>
      </div>
